...
exports.handler = async (event) => {
  ...
  if (!nom || !tel || !email) {
    return { statusCode: 400, body: JSON.stringify({ error: 'Nom, téléphone et email sont requis.' }) };
  }
  // Validation basique — à muscler si besoin (regex email/tel plus strictes,
  // limite de longueur, blocage des doublons email si l'événement l'exige).
  if (nom.length > 120 || email.length > 200 || tel.length > 40) {
